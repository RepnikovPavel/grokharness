...
int cmd_verify(const VerifyOpts& opts)
{
    ...
    for (const auto& step : steps) {
        ...
        if (cr.timed_out) {
            ...
            // Store raw feedback; metrics_encode/TSV layers escape specials.
            r.metrics["feedback"] = feedback;
            // Aider pattern: dump feedback for the agent to fix
            std::fprintf(stderr, "\n======== ghar verify FEEDBACK (%s) ========\n%s\n"
                                 "======== fix this, re-run: ghar verify ========\n\n",
                         step.name.c_str(), feedback.c_str());
        } else {
            r.status = "ok";
            r.exit_code = EXIT_OK;
